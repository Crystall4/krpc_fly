...
iceland 	  = coordinates(name='iceland',        lat=0.00000000000000000, lng=-72.00000000000000000, alt=1000.0)
#======================================================================================================================
...
class fly:
	def __init__(self):
		...
		self.headingPid=pid.PID(self.headingKp,self.headingKi,self.headingKd, -7, 7)
	...
	def pre_take_off(self):
		...
		if vessel.mass>self.MaxTakeoffMass :
			print "exceeded weight."
			return 1
...
testap.TrottlePid.setPoint(160) #для пустого 200 для груженого по максимуму 160.0
testap.maxTrottle = 1.0
testap.minTrottle = 1.0
testap.tALTITUDE = 1000
testap.maxAtDeg = 6.0 #для пустого 7.0 для груженого по максимуму 6.0
testap.minAtDeg = 6.0 #для пустого 7.0 для груженого по максимуму 6.0
old_met=0

print "State; Trottle; HS; AtDeg; Pitch; tVertSp; AtRoll; bearing; tHeading;tAlt;  dist_line;  landingspeed;" 
while state != 'END':
 testap.run()
 ...
 if state == 'Nabor Vblcotbl' and flight.mean_altitude >= testap.tALTITUDE:
	state = 'Recursive dot'
	testap.tALTITUDE =  testap.tLATLONG.alt
	testap.tALTITUDE = 2500
	testap.TrottlePid.setPoint(160) #для пустого 200 для груженого по максимуму 160.0
	testap.maxTrottle = 1.0 #для пустого 0.75 для груженого по максимуму 1.0
	testap.minTrottle = 0.3 #для пустого 0.3 для груженого по максимуму 0.4
	testap.minAtDeg   =-7.0
 ...
 if state == 'Recursive dot' and testap.tLATLONG.dist_line() < 500:
	...
	testap.maxTrottle = 1.0 
	testap.minTrottle = 0.76 #для пустого 0.4 для груженого по максимуму 0.76
 if (state == "Razvorot" and abs(testap.tLATLONG.bearing_line()-flight.heading)<10.0):
	state = 'preRunway' 
	testap.tLATLONG.alt = 150
	testap.tALTITUDE = 150
	testap.TrottlePid.setPoint(150)
	testap.minAtDeg = -1.5 #для пустого -7.0 для груженого по максимуму -1.5
 if state == 'preRunway' and testap.tLATLONG.dist_line() < 1000:
	state = 'Runway'
	testap.tLATLONG = beg_r
	testap.tALTITUDE = 77
	testap.minAtDeg = -0.5 #для пустого -7.0 для груженого по максимуму -0.5
	control.toggle_action_group(9)
	testap.TrottlePid.setPoint(testap.landingspeed())
	testap.maxTrottle = 1.0 #для пустого 0.75 для груженого по максимуму 1.0
	testap.minTrottle = 0.6 #для пустого 0.3 для груженого по максимуму 0.6
 ...
 if state == 'Runway' and testap.tLATLONG.dist_line() < 150:
	...
	testap.tALTITUDE = 74
	testap.minVERTICALSPEED = -0.5
	testap.maxVERTICALSPEED = 1.0
 ...
 print state, ";%.3f;%.1f;%+.3f;%+.3f;%+.3f;%+.3f;%.3f;%.3f;%.1f;%.1f;%.3f;" % (testap.TTrottle, surf_flight.horizontal_speed, testap.AtDeg, flight.pitch, testap.TVertSpeed, testap.AtRoll,testap.tLATLONG.bearing_line(), testap.tHeading, testap.tALTITUDE,  testap.tLATLONG.dist_line(),  testap.landingspeed())
...
print 'Initial bearing >> ', testap.tLATLONG.bearing_line(), '[degrees]'
