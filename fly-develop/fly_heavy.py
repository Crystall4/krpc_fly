...
preRunwayEast = coordinates(name='preRunwayEast',lat=-0.07022509861205912, lng=-72.90451284451766242, alt=150.0)

recursive_dot = coordinates(name='recursive_dot', lat=-0.02022509861205912, lng=-73.18951284451766242, alt=1000.0)
...
if vessel.available_thrust == 0.0 :
	...
	time.sleep(0.2)
control.throttle = 1
while (vessel.thrust) < (vessel.available_thrust * 0.7):
 time.sleep(0.5)
...
while state != 'END':
 ...
 if state == 'flight to RunwayEast' and tLATLONG.dist_line() < 300:
	state = 'Glissada'
	tLATLONG = runwayWest
	tALTITUDE = 60
 ...
 print state, " AtDeg: %.3f ; %.3f  tAlt: %.3f  tVertSp: %.3f dist_line: %.3f bearing_line: %.3f ; %.3f" % (AtDeg, flight.pitch, tALTITUDE, TVertSpeed, tLATLONG.dist_line(), tLATLONG.bearing_line(), tHeading)
