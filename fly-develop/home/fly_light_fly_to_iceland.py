...
while state != 'END':
 ...
 if state == 'preRunway' and testap.tLATLONG.dist_line() < 1000:
	...
	testap.tALTITUDE = beg_r.alt+10
	testap.minAtDeg = -7.0
	testap.TrottlePid.setPoint(testap.aircraft.landingspeed())
	testap.maxTrottle = 1.0
	testap.minTrottle = 0.1
 ...
 print state, ";%.3f;%.1f;%+.3f;%+.3f;%+.3f;%+.3f;%+.3f;%.3f;%.3f;%.1f;%.1f;%.3f;" % (testap.TTrottle, surf_flight.horizontal_speed, testap.AtDeg, flight.pitch, testap.TVertSpeed, surf_flight.vertical_speed, testap.AtRoll,testap.tLATLONG.bearing_line(), testap.tHeading, testap.tALTITUDE,  testap.tLATLONG.dist_line(),  testap.aircraft.landingspeed())
