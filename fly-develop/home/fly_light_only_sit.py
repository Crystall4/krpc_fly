...
icelandEast   = coordinates(name='icelandEast',  lat=-1.5163645187024466,  lng=-71.85121739006479000, alt=135.2)
icelandWest   = coordinates(name='icelandWest',  lat=-1.5177226848000656,  lng=-71.96820722802198000, alt=135.2)
...
class fly:
	...
	def __init__(self):
		...
		self.minVERTICALSPEED = self.aircraft.-10.0
		self.maxVERTICALSPEED = self.aircraft.15.0
		self.workVERTICALSPEED = self.aircraft.15
...
state = 'Recursive dot'
#Select direction
testap.tLATLONG = recursive_dot1
testap.maxTrottle = 1.0
testap.minTrottle = 0.1
testap.tALTITUDE =  500
testap.TrottlePid.setPoint(140)
testap.minAtDeg =-3.0
old_met=0
...
while state != 'END':
 ...
 if state == 'Recursive dot' and testap.tLATLONG.dist_line() < 500:
	state = "Razvorot"
	#Select direction
	if runwayEast.dist_line() < icelandWest.dist_line():
		beg_r = icelandEast
		end_r = icelandWest
 ...
 if (state == "Razvorot" and abs(testap.tLATLONG.bearing_line()-flight.heading)<10.0):
	state = 'preRunway' 
	testap.TrottlePid.setPoint(120)
	testap.tALTITUDE = beg_r.alt+10
 ...
 if state == 'Glissada' and surf_flight.surface_altitude < 5:
	testap.tALTITUDE = beg_r.alt-20
	#print 'Kasanie', tLATLONG.dist_line() 
	testap.TrottlePid.setPoint(20)
	testap.maxTrottle = 0.000000000000000
	testap.minTrottle = 0.000000000000000
	control.brakes = True
 ...
 print state, ";%.3f;%.1f;%+.3f;%+.3f;%+.3f;%+.3f;%+.3f;%.3f;%.3f;%.1f;%.1f;%.3f;" % (testap.TTrottle, surf_flight.horizontal_speed, testap.AtDeg, flight.pitch, testap.TVertSpeed, surf_flight.vertical_speed, testap.AtRoll,testap.tLATLONG.bearing_line(), testap.tHeading, testap.tALTITUDE,  testap.tLATLONG.dist_line(),  testap.landingspeed())
